 ...
 * Style phù hợp với "Sản phẩm bán chạy", "Sản phẩm nổi bật"
 */
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/products/recommendations/home?limit=6`,
          {
            headers: {
        ...
        if (response.data && response.data.code === 200 && response.data.data) {
          ...
          console.warn('No recommendations data:', response.data)
        }
        setError(null)
      } catch (err) {
        console.error('Error loading recommendations:', err)
        // Không hiển thị lỗi, chỉ log để không làm ảnh hưởng UI
        setError(null)
      } finally {
        setLoading(false)
      }
