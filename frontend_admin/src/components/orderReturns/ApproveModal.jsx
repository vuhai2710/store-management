...
const ApproveModal = ({ visible, onCancel, onOk, loading, refundAmount, returnData }) => {
  ...
  const discountPercent = orderTotal > 0 ? ((orderDiscount / orderTotal) * 100).toFixed(1) : 0;
        ...
        <div className="mb-4 p-4 bg-gray-50 rounded border">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{itemsTotal.toLocaleString("vi-VN")} VND</span>
            </div>
            
            {hasDiscount && (
              <div className="flex justify-between text-orange-600">
                <span>Giảm giá đơn hàng ({discountPercent}%):</span>
                <span>- {Math.round(itemsTotal * orderDiscount / orderTotal).toLocaleString("vi-VN")} VND</span>
              </div>
            )}
            
            <div className="border-t pt-2 mt-2">
              <div className="flex justify-between">
                <span className="text-gray-800 font-medium">Số tiền hoàn lại:</span>
