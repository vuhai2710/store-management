    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const categoryId = selectedCategory && selectedCategory !== 'All' 
          ? categories.find(cat => (cat.categoryName || cat.name) === selectedCategory)?.idCategory 
          : null;
        ...
        if (searchTerm) {
          ...
          // Filter by category
          productsData = await productsService.getProductsByCategory(categoryId, {
            pageNo,
            pageSize,
            sortBy,
            sortDirection,
          ...
          // Filter by brand
          productsData = await productsService.getProductsByBrand(selectedBrand, {
            pageNo,
            pageSize,
            sortBy,
            sortDirection,
        ...
        <div style={styles.shopLayout}>
          ...
          <div style={styles.sidebar}>
            ...
            <div style={styles.sidebarSection}>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '1rem', color: '#333' }}>Lọc theo danh mục</h3>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                <li>
                  <button
                      onClick={() => handleCategoryChange('Tất cả')}
                    style={filterButtonStyle(selectedCategory === 'Tất cả' || selectedCategory === 'All')}
                  >
                    Tất cả
                  </button>
                </li>
                {categories.map(cat => (
                  <li key={cat.idCategory || cat.id}>
                    <button
                      onClick={() => handleCategoryChange(cat.categoryName || cat.name)}
                      style={filterButtonStyle(selectedCategory === (cat.categoryName || cat.name))}
                    >
            ...
            {brands.length > 0 && (
              <div style={styles.sidebarSection}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '1rem', color: '#333' }}>Lọc theo thương hiệu</h3>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  <li>
                    <button
                      onClick={() => handleBrandChange('Tất cả')}
                      style={filterButtonStyle(selectedBrand === '' || selectedBrand === 'All' || selectedBrand === 'Tất cả')}
                    >
                      Tất cả
                    </button>
                  </li>
            ...
            <div style={styles.sidebarSection}>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '1rem', color: '#333' }}>Lọc theo giá</h3>
              <div style={{ padding: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <div>
                  <label style={{ fontSize: '0.875rem', color: '#495057', marginBottom: '0.25rem', display: 'block' }}>
                    Giá tối thiểu (₫)
                  </label>
                  <input
                    type="number"
          ...
          <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', padding: '1rem', backgroundColor: 'white', borderRadius: '0.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                <p style={{ color: '#495057', fontWeight: '500' }}>
                    Hiển thị {filteredProducts.length} trong tổng số {totalElements} sản phẩm
                </p>
                
                {/* Sorting Dropdown */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '0.875rem', color: '#6c757d' }}>Sắp xếp theo:</span>
                    <select 
                        value={sortOption}
                        onChange={(e) => {
            ...
            {/* Error Message */}
            {error && (
              <div style={{ padding: '1rem', marginBottom: '1.5rem', backgroundColor: '#f8d7da', color: '#721c24', borderRadius: '0.5rem' }}>
                {error}
              </div>
            )}
